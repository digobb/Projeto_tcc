...
def dados_partida_id(id):
    print(f'ID PARTIDA RETORNADA: {id}')
    dados = retorna_dados_partida(id_partida=id)
    df_dados_partida = pd.json_normalize(dados)
    df_dados_partida.to_csv(f"C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/arquivo/dados_partida_{id}.csv", encoding='utf-8')
    gp(id_partida=id)
    return True
...
def tela_principal():
    ...
    tela.iconbitmap(f'C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/img/icon.ico')
    tela.title('Jogos ao vivo')
    tela.geometry("400x600") 
    tela.minsize(400, 400)
    tela.resizable(False, False)

    img_logo = PhotoImage(file=f'C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/img/campeonato_brasileiro.png', width=150, height=150)
    ...
    if os.path.exists(f'C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/arquivo/jogos_aovivo.csv'):
        print('Arquivo contendo os jogos ao vivo encontrado, verificando...')
        dados = pd.read_csv(f'C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/arquivo/jogos_aovivo.csv')
        if dados.loc[0, 'data_realizacao'] != data_atual:
            print('Atualizando partidas...')
            os.remove(f'C:/Users/{os.getlogin()}/Documents/Projeto_TCC/projeto_tcc/arquivo/jogos_aovivo.csv')
            time.sleep(1)
